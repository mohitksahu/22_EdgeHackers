...
function TypewriterText({ text, isTyping, onComplete }) {
  ...
  useEffect(() => {
    ...
    const delay = char === ' ' ? 5 : char === '.' || char === ',' ? 30 : 10;
  ...
  return <>{displayedText}{isTyping && currentIndex < text.length && <span className="animate-pulse">▊</span>}</>;
}

// Render markdown-like content
function RenderContent({ content, isTyping }) {
    ...
    <div className="text-[var(--text-color,#e5e7eb)] text-sm leading-relaxed space-y-2">
      {lines.map((line, idx) => {
        ...
        if (line.startsWith('•') || line.startsWith('-')) {
          const bulletContent = line.startsWith('•') ? line.substring(1).trim() : line.substring(1).trim();
          return (
            <div key={idx} className="flex items-start gap-2 ml-2">
              <span className="text-pluto-purple mt-0.5">•</span>
        ...
        if (line.startsWith('**Sources:**') || line.startsWith('Sources:')) {
          return (
            <p key={idx} className="font-semibold text-pluto-purple mt-4 pt-3 border-t border-[var(--card-border,#1f1f2e)]">
              📚 Sources
            </p>
          );
        }
      ...
      {isTyping && <span className="animate-pulse text-pluto-purple">▊</span>}
    </div>
  );
}
                  ...
                  <div className="max-w-3xl">
                    <div className="flex items-start gap-3">
                      <div className="w-8 h-8 rounded-full bg-[var(--card-bg,#14141f)] border border-[var(--card-border,#1f1f2e)] flex items-center justify-center flex-shrink-0">
                        <svg className={`w-4 h-4 text-pluto-purple ${message.isTyping ? 'animate-pulse' : ''}`} fill="currentColor" viewBox="0 0 20 20">
                          <path d="M10 3.5a1.5 1.5 0 013 0V4a1 1 0 001 1h3a1 1 0 011 1v3a1 1 0 01-1 1h-.5a1.5 1.5 0 000 3h.5a1 1 0 011 1v3a1 1 0 01-1 1h-3a1 1 0 01-1-1v-.5a1.5 1.5 0 00-3 0v.5a1 1 0 01-1 1H6a1 1 0 01-1-1v-3a1 1 0 00-1-1h-.5a1.5 1.5 0 010-3H4a1 1 0 001-1V6a1 1 0 011-1h3a1 1 0 001-1v-.5z" />
                        </svg>
                      </div>
                      <div className="flex-1">
                        <div className={`bg-[var(--card-bg,#14141f)]/50 backdrop-blur-glass rounded-2xl px-5 py-4 border border-[var(--card-border,#1f1f2e)] ${message.isTyping ? 'border-pluto-purple/30' : ''} transition-all duration-300`}>
                          {/* Response Content with Typing Effect */}
                          <RenderContent 
                            content={message.content} 
                            isTyping={message.isTyping}
                          />

                          {/* Processing Time - Show after typing */}
                          {!message.isTyping && message.processingTime && (
                            <div className="mt-3 text-xs text-[var(--secondary-text,#9ca3af)] italic animate-fadeIn">
                              ⚡ Processed in {message.processingTime}s
                            </div>
                          )}

                          {/* Source Citations - Show after typing */}
                          {!message.isTyping && message.citations && message.citations.length > 0 && (
                            <div className="flex flex-wrap gap-2 mt-4 pt-4 border-t border-[var(--card-border,#1f1f2e)] animate-fadeIn">
            ...
            {isLoading && (
              <div className="flex justify-start">
                <div className="max-w-3xl">
                  <div className="flex items-start gap-3">
                    <div className="w-8 h-8 rounded-full bg-[var(--card-bg,#14141f)] border border-[var(--card-border,#1f1f2e)] flex items-center justify-center flex-shrink-0">
                      <svg className="w-4 h-4 text-pluto-purple animate-pulse" fill="currentColor" viewBox="0 0 20 20">
                        <path d="M10 3.5a1.5 1.5 0 013 0V4a1 1 0 001 1h3a1 1 0 011 1v3a1 1 0 01-1 1h-.5a1.5 1.5 0 000 3h.5a1 1 0 011 1v3a1 1 0 01-1 1h-3a1 1 0 01-1-1v-.5a1.5 1.5 0 00-3 0v.5a1 1 0 01-1 1H6a1 1 0 01-1-1v-3a1 1 0 00-1-1h-.5a1.5 1.5 0 010-3H4a1 1 0 001-1V6a1 1 0 011-1h3a1 1 0 001-1v-.5z" />
                      </svg>
                    </div>
                    <div className="flex-1">
                      <div className="bg-[var(--card-bg,#14141f)]/50 backdrop-blur-glass rounded-2xl px-5 py-4 border border-pluto-purple/30">
                        <div className="flex items-center gap-3">
                          <div className="flex items-center gap-1">
                            <div className="w-2 h-2 bg-pluto-purple rounded-full animate-bounce" style={{ animationDelay: '0ms' }}></div>
                            <div className="w-2 h-2 bg-pluto-purple rounded-full animate-bounce" style={{ animationDelay: '150ms' }}></div>
                            <div className="w-2 h-2 bg-pluto-purple rounded-full animate-bounce" style={{ animationDelay: '300ms' }}></div>
                          </div>
                          <span className="text-sm text-[var(--secondary-text,#9ca3af)]">Thinking...</span>
                        </div>
                      </div>
                    </div>
                  </div>
          ...
          <div className="px-8 pb-8">
            <div className="space-y-6">
              ...
              <div className="flex justify-end gap-3">
                <button onClick={handleRenameCancel} className="px-4 py-2.5 text-sm font-medium text-[var(--text-color)] bg-transparent border border-[var(--card-border,#1f1f2e)] rounded-lg hover:bg-[var(--card-border,#1f1f2e)]/50 transition-colors">
                  Cancel
                </button>
                <button
