...
import { resetVectorStore } from '../../services/apiClient';
...
export default function Dashboard() {
  ...
  useEffect(() => {
    if (location.state?.loadPlanet) {
      ...
      // Clear embeddings for new planet to prevent interference from old documents
      console.log('Resetting vector store for new planet...');
      resetVectorStore().then(() => {
        console.log('Vector store reset completed');
      }).catch(error => {
        console.error('Failed to reset vector store for new planet:', error);
      });
    }
  }, [location.state]);

  // Save planet history in real-time when messages or sources change
  useEffect(() => {
    if (messages.length > 0) {
      ...
      // Update existing planet or add new one
    ...
    try {
      ...
      const aiMessage = {
        ...
        citations: (response.sources || []).map(source => 
          typeof source === 'string' ? source : (source.file_name || source.filename || 'Unknown')
        ),
      ...
      setTimeout(() => {
        // FIX 1.4: Update evidence with per-source confidence (deduplicated at source level)
        // Backend sends 'sources' array (unique filenames) and 'confidence_per_source' object
        const sourcesArray = response.sources || [];
        const confidencePerSource = response.confidence_per_source || {};
        
        // FIX 1.4: Render sources ONCE per file (not per chunk)
        const evidenceCards = sourcesArray.map((filename, index) => {
          // Use per-source confidence from backend (convert 0-1 to 0-100)
          const confidenceRaw = confidencePerSource[filename] ?? response.confidence_score ?? 0;
          ...
          return {
            ...
            excerpt: '',  // No chunk-level data in frontend
            modality: 'text',  // Infer from extension if needed
            fileType: 'text'
          };
        });

        // Use global confidence from stable response contract (convert 0-1 to 0-100)
        const globalConfidence = Math.round((response.confidence_score ?? response.confidence ?? 0) * 100);

        setEvidenceData({
          confidence: globalConfidence,
          sources: sourcesArray.length,
          cards: evidenceCards,
          conflictsDetected: response.conflicts_detected || false,
